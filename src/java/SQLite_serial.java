...
public class SQLite_serial {
  public static void main(String[] args) {
    ...
    try {
      ...
      stmt.executeUpdate("DROP TABLE IF EXISTS tb_java_serial; CREATE TABLE tb_java_serial(id INTEGER PRIMARY KEY " +
          "AUTOINCREMENT, z_score REAL NOT NULL, cumulative_distribution REAL NOT NULL)");
      for (i = -500; i <= 500; i++) {
        x = (double) i / 100.0;
        prob = Gaussian.gaussianCDF(0, 1, x);
        stmt.executeUpdate(
            String.format("INSERT INTO tb_java_serial(z_score, cumulative_distribution) VALUES (%.2f, %f)", x, prob));
      }
    } catch (Exception e) {
